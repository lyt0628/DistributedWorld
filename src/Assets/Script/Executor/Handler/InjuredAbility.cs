...
namespace QS.Chara.Domain.Handler
{
    ...
    /// </summary>
    public class InjuredAbility : AbstractHandler
    {

        readonly IInjurable injurable;

        public InjuredAbility(IRelayExecutor executor, IInjurable injurable)
            :base(executor)
        {
        ...
        {
            if (!ReflectionUtil.IsChildOf<IInjuredInstr>(msg))
            {
                context.Write(msg);
                return;
            }
            
            var i = (IInjuredInstr)msg;
